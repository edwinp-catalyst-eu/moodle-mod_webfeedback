...
function xmldb_webfeedback_upgrade($oldversion=0) {
...
/// for demonstrative purposes and to show how the webfeedback
/// iself has been upgraded.

/// For each upgrade block, the file webfeedback/version.php
...
/// First example, some fields were added to the module on 20070400
    ...
    /// Define field course to be added to webfeedback
        $table = new XMLDBTable('webfeedback');
        $field = new XMLDBField('course');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0', 'id');
    /// Launch add field course
        $result = $result && add_field($table, $field);

    /// Define field intro to be added to webfeedback
        $table = new XMLDBTable('webfeedback');
    ...
    /// Define field introformat to be added to webfeedback
        $table = new XMLDBTable('webfeedback');
...
/// "01" in the last two digits of the version
    ...
    /// Define field timecreated to be added to webfeedback
        $table = new XMLDBTable('webfeedback');
    ...
    /// Define field timemodified to be added to webfeedback
        $table = new XMLDBTable('webfeedback');
    ...
    /// Define index course (not unique) to be added to webfeedback
        $table = new XMLDBTable('webfeedback');
...
/// Third example, the next day, 20070402 (with the trailing 00), some inserts were performed, related with the module
    ...
    /// Add some actions to get them properly displayed in the logs
        $rec = new stdClass;
        $rec->module = 'webfeedback';
        $rec->action = 'add';
        $rec->mtable = 'webfeedback';
        $rec->filed  = 'name';
